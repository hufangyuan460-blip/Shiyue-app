...
> 实现记录（2026-08-06）：Room 升级为 v3，新增 `reading_sessions` 和单行 `active_reading_session` 活动槽位，并提供显式 `Migration(2, 3)` 与 Schema。领域层同时保存 UTC 毫秒墙上时间和 Android 单调时钟锚点：墙上时间用于历史展示，`elapsedRealtime` 用于计算活动段，暂停时固化累计有效时长。进程重建后从 Room 恢复；检测到设备重启、墙上时间回退、双时钟分歧超过 2 分钟或连续活动超过 12 小时时进入人工恢复。本阶段刻意不引入前台服务、WorkManager、WakeLock 或通知，因为无需在后台逐秒执行任务，持久化锚点即可在恢复时精确结算正常场景，也避免额外耗电和权限负担。
