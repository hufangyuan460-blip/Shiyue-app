...
已完成阶段 0 工程骨架、阶段 1“书架 1.0”和阶段 2“阅读闭环 1.0”。Room v3 通过显式 `Migration(2, 3)` 增加阅读场次与唯一活动槽位，Schema 1/2/3 均受版本管理。计时以 UTC 墙上时钟持久化起止时间，以 `elapsedRealtime` 计算当前有效时段；暂停时间不计入，锁屏、后台和进程重建后可从持久化锚点恢复。检测到重启、系统时间回退、双时钟明显分歧或异常长时段时，不静默计入不可信时间，而是进入恢复页面供用户选择。
