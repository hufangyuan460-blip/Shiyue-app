...
当前已完成书架 1.0 和阅读闭环 1.0：可从单本书开始阅读，暂停/继续、结束并更新页码，异常时恢复或补录有效时长，并在书籍详情查看、修正或删除已完成场次。书架与阅读数据均离线保存在 Room；网络、ISBN、随记、目标和统计仍未实现。
...
已完成阶段 0 工程骨架、阶段 1“书架 1.0”和阶段 2“阅读闭环 1.0”。Room v3 通过显式 `Migration(2, 3)` 增加阅读场次与唯一活动槽位，Schema 1/2/3 均受版本管理。计时以 UTC 墙上时钟持久化起止时间，以 `elapsedRealtime` 计算当前有效时段；暂停时间不计入，锁屏、后台和进程重建后可从持久化锚点恢复。检测到重启、系统时间回退、双时钟明显分歧或异常长时段时，不静默计入不可信时间，而是进入恢复页面供用户选择。书架卡片展示每本书“今日 X · 累计 Y”阅读时长，只统计已完成场次，跨午夜按结束时间归入当天，无需新增数据库字段。回顾页展示 App 累计阅读时长、今日/本周/本月、当月阅读日历热力图、连续/累计阅读天数、读完书籍数与阅读时长最多的书；同样无需新增数据库字段。

### 阅读闭环说明

- 同一时间只允许一场活动阅读；开始、暂停、继续、结束、放弃均通过 Repository 事务写入 Room。
- 计时页面的每秒刷新只更新展示，不逐秒写数据库；不使用前台服务、WorkManager、WakeLock 或通知。
- 结束小结可选择是否同步页码；回读和读到最后一页均需明确确认。
- 已完成场次进入书籍详情的阅读轨迹，可查看汇总、修正时间/时长/页码或删除记录；放弃场次不进入统计。
- 连接设备后运行 `connectedDebugAndroidTest` 可验证生产导航的开始→计时→结束→轨迹闭环；锁屏、系统回收、重启与系统改时仍需真机人工验收。
