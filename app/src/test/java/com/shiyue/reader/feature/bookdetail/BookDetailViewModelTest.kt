...
class BookDetailViewModelTest {
    ...
    fun `loads content and follows repository updates`() = runTest {
        ...
        runCurrent()
        assertEquals(original, (viewModel.uiState.value as BookDetailUiState.Content).book)

        val updated = original.updated(title = "新书名", updatedAt = 2)
        repository.updateBook(updated)
        runCurrent()
        assertEquals(updated, (viewModel.uiState.value as BookDetailUiState.Content).book)
    }
